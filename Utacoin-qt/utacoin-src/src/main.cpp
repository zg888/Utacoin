...
uint256 hashGenesisBlock("0x00000cb5c8b9be163f2fc8651d72c215f2c6dd2b2299e8c4a1f66a72653868ed");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // UtaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "UtaCoin Signed Message:\n";
...
{
    // UtaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // UtaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
	...
	// Mainnet Genesis block: 
		...
		const char* pszTimestamp = "UtaCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xaa, 0xc3, 0xb4, 0xde }; // UtaCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UtaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UtaCoinMiner : ProcessBlock, block not accepted");
    }
